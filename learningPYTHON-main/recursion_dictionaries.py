...
def lyrics_to_frequencies(lyrics):
    ...
    -------
    myDict : Dictionary
        A dictionary with the song lyrics and their frequencies.

    """
    myDict = {} # initialize an empty dictionary
    for word in lyrics: # loop over the list parameter
        if word in myDict:
            myDict[word] += 1 # increment its count
        else:
            myDict[word] = 1 # add it for for first time
    return myDict
...
def most_common_words(freqs):
    ...
    ----------
    freqs : Dictionary
        A dictionary of lyrics and their frequencies.

    Returns
    -------
    words : List
        List of the most frequent word.
    best : int
        frequency of the most common word.

    """
    values = freqs.values() # get values in the dict
    best = max(values) # get max value
    words = [] # an empty list
    for k in freqs:
        if freqs[k] == best:
            words.append(k)
...
def words_often(freqs, minTimes):
    ...
    minTimes : int
        minimum frequency tested for.

    Returns
    -------
    result : List
        List of tuples with most often words.
